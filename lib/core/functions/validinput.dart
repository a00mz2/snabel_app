String? validInput(
  ...
  // تحقق الرقم: محلي 07xxxxxxxxx (11) أو دولي 9647xxxxxxxxx (13) كما يخزّنه السيرفر
  if (type == "phone" && val.isNotEmpty) {
    final digits = val.replaceAll(RegExp(r'\D'), '');
    if (min > 0 && digits.length < min) {
      return "لا يمكن ان يكون حقل الرقم اقل من $min رقم";
    }
    if (RegExp(r'^9647\d{9}$').hasMatch(digits)) {
      // 13 رقماً بصيغة العراق الدولية
    } else if (RegExp(r'^07\d{9}$').hasMatch(digits)) {
      // 11 رقماً محلياً
    } else {
      return "رقم غير صالح: 07xxxxxxxxx أو 9647xxxxxxxxx";
    }
  }
